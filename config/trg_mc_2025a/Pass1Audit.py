...
files = list(mc_client.query(query=query,with_metadata=True, with_provenance=True))
...
dataset = files[0]['metadata']['merge.dataset']
print ("dataset",dataset)
inputlist = open("inputfiles.txt",'w')
query = "files from %s "%dataset
print (query)
inputs = mc_client.query(query=query)
for file in inputs:
    inputlist.write(file['fid'] + "\n")
inputlist.close()
